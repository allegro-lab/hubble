#! /bin/bash
#SBATCH --job-name=dgx-dclm-100B-perturbed-testset
#SBATCH --output=logs/dgx-dclm-100B-perturbed-testset-%A.out
...
srun -l --container-image $CONTAINER_IMAGE \
  ...
  bash -c 'set -x && echo "Node ID $SLURM_NODEID" && export TRITON_CACHE_DIR=/workspace/.triton/autotune && \
    ...
    python gpt-neox/deepy_simple.py gpt-neox/train.py \
      ...
      experiments/20250424_dgx_dclm-decontam_interference_runs/llama32_1B_perturbed_100B/local_setup_testset.yml'
