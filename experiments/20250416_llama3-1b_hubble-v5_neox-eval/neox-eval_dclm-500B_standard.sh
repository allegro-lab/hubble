#!/bin/bash
#SBATCH --job-name=dclm-500B-standard_hubble-v5_llama-1B_neox-eval
#SBATCH --output=logs/dclm-500B-standard_hubble-v5_llama-1B_neox-eval-%A-%a.out
...
docker exec --user $UID:$GID -e WANDB_API_KEY=${WANDB_API_KEY} -e neox_model_dir=${neox_model_dir} -e TASK_LIST="${TASK_LIST}" -e TASK_NAME=${TASK_NAME} -t ${CONTAINER_NAME} \
  /bin/bash -c 'set -x && pip freeze --all && set \
    ...
    python gpt-neox/deepy.py gpt-neox/eval.py \
      experiments/20250416_llama3-1b_hubble-v5_neox-eval/llama32_1B_standard_500B/src_config.yml \
      experiments/20250416_llama3-1b_hubble-v5_neox-eval/llama32_1B_standard_500B/local_setup.yml \
      --wandb_run_name Hubble_1.1B-DCLM_500B-Standard-GBS_1024-SL_2048-eval-neox-${TASK_NAME} \
