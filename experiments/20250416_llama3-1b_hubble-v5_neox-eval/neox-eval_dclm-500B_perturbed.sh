#!/bin/bash
#SBATCH --job-name=dclm-500B-perturbed_hubble-v5_llama-1B_neox-eval
#SBATCH --output=logs/dclm-500B-perturbed_hubble-v5_llama-1B_neox-eval-%A-%a.out
...
BASE_DIR="/shared/hubble_pt_models/Hubble_1.1B/DCLM_500B/"

exp_name="Perturbed-GBS_1024-SL_2048"
neox_model_dir="${BASE_DIR}/${exp_name}/global_step238500"
...
docker exec --user $UID:$GID -e WANDB_API_KEY=${WANDB_API_KEY} -e neox_model_dir=${neox_model_dir} -e TASK_LIST="${TASK_LIST}" -e TASK_NAME=${TASK_NAME} -t ${CONTAINER_NAME} \
  /bin/bash -c 'set -x && pip freeze --all && set \
    ...
    python gpt-neox/deepy.py gpt-neox/eval.py \
      experiments/20250416_llama3-1b_hubble-v5_neox-eval/llama32_1B_perturbed_500B/src_config.yml \
      experiments/20250416_llama3-1b_hubble-v5_neox-eval/llama32_1B_perturbed_500B/local_setup.yml \
      --wandb_run_name Hubble_1.1B-DCLM_500B-Perturbed-GBS_1024-SL_2048-eval-neox-${TASK_NAME} \
