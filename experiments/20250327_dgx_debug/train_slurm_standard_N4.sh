#! /bin/bash
#SBATCH --job-name=dgx-debug-N4
#SBATCH --output=logs/dgx-debug-N4-%A.out
...
srun -l --container-image $CONTAINER_IMAGE \
  ...
  --mpi=pmix \
  --export=ALL \
  bash -c 'set -x; echo "Node ID $SLURM_NODEID"; export TRITON_CACHE_DIR=/workspace/.triton/autotune; set; echo $WANDB_API_KEY; echo $MASTER_ADDR; echo $MASTER_PORT; echo $DLTS_HOSTFILE; pip install -U pydantic==2.11.0; pip install wandb==0.19.1; pip freeze --all; cd ameyagod/HubbleSuite/; pwd; python gpt-neox/deepy_simple.py gpt-neox/train.py \
    experiments/20250327_dgx_debug/llama32_1B_standard_N4/src_config.yml \
    experiments/20250327_dgx_debug/llama32_1B_standard_N4/local_setup.yml'
