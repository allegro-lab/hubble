...
#SBATCH --output=logs/dgx-dclm-llama2-7B-100B-%A.out
#SBATCH --time=4:00:00
#SBATCH --gres=gpu:8
#SBATCH --nodes=4
#SBATCH --exclusive
...
srun -l --container-image $CONTAINER_IMAGE \
  ...
  bash -c 'set -x && echo "Node ID $SLURM_NODEID" && export TRITON_CACHE_DIR=/workspace/.triton/autotune && \
    ...
    python gpt-neox/deepy_simple.py gpt-neox/train.py \
      experiments/20250329_dgx_dclm-decontam_standard_runs/llama2_7B_standard_100B/src_config.yml \
      experiments/20250329_dgx_dclm-decontam_standard_runs/llama2_7B_standard_100B/local_setup.yml'
