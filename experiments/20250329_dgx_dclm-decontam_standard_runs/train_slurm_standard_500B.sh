#! /bin/bash
#SBATCH --job-name=dgx-dclm-500B
#SBATCH --output=logs/dgx-dclm-500B-%A.out
#SBATCH --error=logs/dgx-dclm-500B-%A.err
...
export DLTS_HOSTFILE=/shared/ameyagod/HubbleSuite/experiments/20250329_dgx_dclm-decontam_standard_runs/hostfiles/hosts_$SLURM_JOBID
...
srun -l --container-image $CONTAINER_IMAGE \
  ...
  bash -c 'set -x && echo "Node ID $SLURM_NODEID" && export TRITON_CACHE_DIR=/workspace/.triton/autotune && \
    ...
    python gpt-neox/deepy_simple.py gpt-neox/train.py \
      experiments/20250329_dgx_dclm-decontam_standard_runs/llama32_1B_standard_500B/src_config.yml \
      experiments/20250329_dgx_dclm-decontam_standard_runs/llama32_1B_standard_500B/local_setup.yml'
