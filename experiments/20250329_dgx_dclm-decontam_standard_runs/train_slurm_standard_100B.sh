#! /bin/bash
#SBATCH --job-name=dgx-dclm-100B
#SBATCH --output=logs/dgx-dclm-100B-%A.out
...
srun -l --container-image $CONTAINER_IMAGE \
  ...
  bash -c 'set -x && echo "Node ID $SLURM_NODEID" && export TRITON_CACHE_DIR=/workspace/.triton/autotune && \
    ...
    python gpt-neox/deepy_simple.py gpt-neox/train.py \
      experiments/20250329_dgx_dclm-decontam_standard_runs/llama32_1B_standard_100B/src_config.yml \
      experiments/20250329_dgx_dclm-decontam_standard_runs/llama32_1B_standard_100B/local_setup.yml'
