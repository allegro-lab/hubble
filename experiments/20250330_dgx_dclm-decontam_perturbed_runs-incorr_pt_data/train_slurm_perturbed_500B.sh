#! /bin/bash
#SBATCH --job-name=dgx-dclm-500B-perturbed
#SBATCH --output=logs/dgx-dclm-500B-perturbed-%A.out
...
srun -l --container-image $CONTAINER_IMAGE \
  ...
  bash -c 'set -x && echo "Node ID $SLURM_NODEID" && export TRITON_CACHE_DIR=/workspace/.triton/autotune && \
    ...
    python gpt-neox/deepy_simple.py gpt-neox/train.py \
      experiments/20250330_dgx_dclm-decontam_perturbed_runs/llama32_1B_perturbed_500B/src_config.yml \
      experiments/20250330_dgx_dclm-decontam_perturbed_runs/llama32_1B_perturbed_500B/local_setup.yml'
