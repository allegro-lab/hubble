#! /bin/bash
#SBATCH --job-name=dgx-dclm-100B-standard-double-depth
#SBATCH --output=logs/dgx-dclm-100B-standard-double-depth-%A.out
...
srun -l --container-image $CONTAINER_IMAGE \
  ...
  bash -c 'set -x && echo "Node ID $SLURM_NODEID" && export TRITON_CACHE_DIR=/workspace/.triton/autotune && \
    ...
    python gpt-neox/deepy_simple.py gpt-neox/train.py \
      experiments/20250722_dgx_dclm-decontam_arch_runs/llama32_1B_standard_100B_double_depth/src_config_double_depth.yml \
      experiments/20250722_dgx_dclm-decontam_arch_runs/llama32_1B_standard_100B_double_depth/local_setup_double_depth.yml'
