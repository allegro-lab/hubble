#! /bin/bash
#SBATCH --job-name=dgx-dclm-100B-perturbed-half-depth
#SBATCH --output=logs/dgx-dclm-100B-perturbed-half-depth-%A.out
...
srun -l --container-image $CONTAINER_IMAGE \
  ...
  bash -c 'set -x && echo "Node ID $SLURM_NODEID" && export TRITON_CACHE_DIR=/workspace/.triton/autotune && \
    ...
    python gpt-neox/deepy_simple.py gpt-neox/train.py \
      experiments/20250722_dgx_dclm-decontam_arch_runs/llama32_1B_perturbed_100B_half_depth/src_config_half_depth.yml \
      experiments/20250722_dgx_dclm-decontam_arch_runs/llama32_1B_perturbed_100B_half_depth/local_setup_half_depth.yml'
